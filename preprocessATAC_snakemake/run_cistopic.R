...
write.csv(p, paste0(tools::file_path_sans_ext(countpath), ".predDist.csv"), quote=FALSE)
...
gene_mat <- t(adj_mat[,keep.genes]) %*% p
gene_mat <- t(gene_mat)
                   
write.csv(as.matrix(gene_mat), paste0(tools::file_path_sans_ext(countpath), ".geneScores.csv"), quote=FALSE)
